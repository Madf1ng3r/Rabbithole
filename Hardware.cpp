...
int hwmain()
{
    ...
    if (!bstrPath)
    {
        std::wcerr << L"Failed to allocate BSTR for the path." << std::endl;
        pLoc->Release();
        CoUninitialize();
        return 1;
    }
    ...
    if (FAILED(hres))
    {
        std::wcerr << L"Could not connect. Error code = 0x" << std::hex << hres << std::endl;
        pLoc->Release();
        CoUninitialize();
        return 1; // Program has failed.
    }
    ...
    if (FAILED(hres))
    {
        std::wcerr << L"Query for operating system name failed. Error code = 0x" << std::hex << hres << std::endl;
        pSvc->Release();
        pLoc->Release();
        CoUninitialize();
        return 1; // Program has failed.
